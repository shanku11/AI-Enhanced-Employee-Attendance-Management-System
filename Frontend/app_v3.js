...
let emailJsSettings = null;
let latestWarningPreview = null;
  ...
  tabBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (targetTab === "adminTrendsTab") {
        loadTrendsData();
      } else if (targetTab === "adminUploadTab") {
        // Upload tab is event-driven.
...
async function loadAllEmployeesList() {
  try {
    ...
    const warningSelector = document.getElementById("warningEmployeeSelector");
    if (warningSelector) warningSelector.innerHTML = "";
    ...
    employees.forEach(emp => {
      ...
      if (warningSelector) {
        const warningOption = option.cloneNode(true);
        warningSelector.appendChild(warningOption);
      }
    });
...
async function runPredictiveAnalytics(userId) {
  ...
  try {
    ...
    alert("Analytics load error. Ensure your FastAPI Backend is running at https://ai-enhanced-employee-attendance.onrender.com");
  }
}
...
// ----------------- COMPANY CSV UPLOAD & LABELING -----------------
function setupCompanyCsvUpload() {
  const form = document.getElementById("companyCsvUploadForm");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const body = document.getElementById("companyCsvResultsBody");
    ...
    body.innerHTML = `<tr><td colspan="8" style="text-align:center;"><div class="loader-spinner"></div>Classifying uploaded employees...</td></tr>`;
    summary.textContent = "Processing";

    try {
      const res = await fetch(`${API_BASE_URL}/api/company/upload-csv`, {
        method: "POST",
        body: formData
      });
      const data = await res.json();
      ...
      summary.textContent = `${data.processed} rows labeled`;
      body.innerHTML = "";

      if (!data.results.length) {
        body.innerHTML = `<tr><td colspan="8" style="text-align:center; color:var(--text-muted);">No valid rows found. Check the template headers.</td></tr>`;
        return;
      }

      data.results.forEach(item => {
        const predictionClass = item.prediction.includes("Regular") ? "regular" : item.prediction.includes("Irregular") ? "irregular" : "at-risk";
        const row = document.createElement("tr");
        row.innerHTML = `
          <td style="font-weight:600;">${item.name || item.employee_id || "Unknown"}</td>
          <td>${item.email || "-"}</td>
          <td>${item.department || "-"}</td>
          <td>${item.absences}</td>
          <td>${item.late_count}</td>
          <td>${item.attendance_rate}%</td>
          <td><span class="prediction-mini ${predictionClass}">${item.prediction}</span></td>
      ...
      if (data.errors && data.errors.length > 0) {
        showToastAlert("CSV partially processed", `${data.errors.length} row(s) had validation errors.`, "medium");
      }
    } catch (err) {
      summary.textContent = "Upload failed";
      body.innerHTML = `<tr><td colspan="8" style="text-align:center; color:var(--danger);">${err.message}</td></tr>`;
    }
  });
}

// ----------------- SETTINGS & EMAILJS WARNING MAIL -----------------
function setupEmailSettings() {
  const form = document.getElementById("emailJsSettingsForm");
  const previewBtn = document.getElementById("previewWarningEmailBtn");
  const sendBtn = document.getElementById("sendWarningEmailBtn");
  ...
  if (form && form.dataset.bound !== "true") {
    ...
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      const payload = {
        enabled: document.getElementById("emailJsEnabled").value === "true",
        service_id: document.getElementById("emailJsServiceId").value.trim(),
        template_id: document.getElementById("emailJsTemplateId").value.trim(),
        public_key: document.getElementById("emailJsPublicKey").value.trim(),
        reply_to: document.getElementById("emailJsReplyTo").value.trim() || "hr@company.com",
        from_name: "HR Attendance Team"
      };

      try {
        const res = await fetch(`${API_BASE_URL}/api/settings/emailjs`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
        ...
        emailJsSettings = data.settings;
        alert("EmailJS settings updated.");
      } catch (err) {
  ...
  if (previewBtn && previewBtn.dataset.bound !== "true") {
    ...
    previewBtn.addEventListener("click", previewWarningEmail);
  }

  if (sendBtn && sendBtn.dataset.bound !== "true") {
    sendBtn.dataset.bound = "true";
    sendBtn.addEventListener("click", sendWarningEmail);
  }
...
async function loadEmailSettings() {
  try {
    ...
    if (!res.ok) throw new Error("Could not load EmailJS settings");

    emailJsSettings = settings;
    document.getElementById("emailJsEnabled").value = String(Boolean(settings.enabled));
    document.getElementById("emailJsServiceId").value = settings.service_id || "";
    document.getElementById("emailJsTemplateId").value = settings.template_id || "";
    document.getElementById("emailJsPublicKey").value = settings.public_key || "";
    document.getElementById("emailJsReplyTo").value = settings.reply_to || "hr@company.com";
  } catch (err) {
    console.error("Email settings load error:", err);
  }
}

async function previewWarningEmail() {
  const employeeId = parseInt(document.getElementById("warningEmployeeSelector").value);
  const message = document.getElementById("warningMessageText").value.trim();
  const previewBox = document.getElementById("warningEmailPreview");
  if (!employeeId) return;

  previewBox.innerHTML = `<div class="loading-container"><div class="loader-spinner"></div>Generating warning preview...</div>`;

  try {
    const res = await fetch(`${API_BASE_URL}/api/warnings/email-preview`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ employee_id: employeeId, message: message || null })
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || "Could not generate warning preview");

    latestWarningPreview = data;
    previewBox.innerHTML = `
      <p><strong>To:</strong> ${data.to_name} &lt;${data.to_email}&gt;</p>
      <p><strong>Subject:</strong> ${data.subject}</p>
      <p><strong>ML Status:</strong> ${data.prediction} (${data.confidence}% confidence)</p>
      <p style="margin-top:10px;">${data.message}</p>
    `;
  } catch (err) {
    previewBox.innerHTML = `<span style="color:var(--danger);">${err.message}</span>`;
  }
}

async function sendWarningEmail() {
  if (!latestWarningPreview) {
    await previewWarningEmail();
  }
  if (!latestWarningPreview) return;

  if (!emailJsSettings || !emailJsSettings.enabled) {
    alert("Enable EmailJS in Settings before sending warning mail.");
    return;
  }
  if (!emailJsSettings.service_id || !emailJsSettings.template_id || !emailJsSettings.public_key) {
    alert("Service ID, Template ID, and Public Key are required.");
    return;
  }
  if (!window.emailjs) {
    alert("EmailJS SDK is not loaded. Check your internet connection and reload the page.");
    return;
  }

  try {
    window.emailjs.init({ publicKey: emailJsSettings.public_key });
    await window.emailjs.send(emailJsSettings.service_id, emailJsSettings.template_id, {
      to_name: latestWarningPreview.to_name,
      to_email: latestWarningPreview.to_email,
      employee_id: latestWarningPreview.employee_id,
      department: latestWarningPreview.department,
      subject: latestWarningPreview.subject,
      message: latestWarningPreview.message,
      prediction: latestWarningPreview.prediction,
      confidence: latestWarningPreview.confidence,
      from_name: emailJsSettings.from_name || "HR Attendance Team",
      reply_to: emailJsSettings.reply_to || "hr@company.com"
    });
    alert("Warning email sent through EmailJS.");
  } catch (err) {
    console.error("EmailJS send error:", err);
    alert("EmailJS send failed. Check service/template IDs and allowed template variables.");
  }
}
